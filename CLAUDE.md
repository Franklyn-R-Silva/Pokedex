...
A single-page Pokédex that fetches data from the public [PokéAPI](https://pokeapi.co/) and lets the user search by name/number and page through Pokémon with Prev/Next buttons (and arrow keys). Built with vanilla JavaScript ES modules, bundled with **Vite**. Live site: https://pokedex.devfrs.com/ (deployed on Cloudflare Pages; build `npm run build`, output `dist`). Unknown routes fall back to `public/404.html`. Bilingual UI (PT-BR/EN) via `src/i18n.js`; in PT mode, API text (description/genus/abilities) uses Spanish as the closest approximation since PokéAPI has no Portuguese.
...
- `src/i18n.js` — PT-BR/EN dictionaries. `t(key)` returns UI strings for the current language; `contentLang()` maps the UI language to the API content language (`pt`→`es`, `en`→`en`). Language persists in `localStorage` and toggles via the `.lang-toggle` button; static text uses `data-i18n`/`data-i18n-ph` attributes applied by `applyStaticI18n()`.
- `src/filter.js` — `setupFilter(...)` renders a clickable grid filtered by type and/or generation (intersecting by id when both are set). Returns `{ refresh }` to re-label on language change.
- `src/compare.js` — `setupCompare(...)` manages a list of up to **4 Pokémon** (removable chips) and renders an **SVG radar chart** (6 stats, one polygon per Pokémon) plus an **N-column stat table** highlighting the max per row and the total. Exposes `add(name)` (called by the autocomplete's `onSelect`) and `refresh`.
