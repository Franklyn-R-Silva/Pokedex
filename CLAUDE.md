...
- `src/compare.js` — `setupCompare(...)` fetches two Pokémon and renders a side-by-side table (types, height, weight, stats, total) highlighting the higher value. Returns `{ refresh }`.
